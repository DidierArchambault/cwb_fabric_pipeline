...
def _load_cleaning_specs(config: Configuration) -> Dict:
    """JSON cleaning rules loader."""
    cleaning_rules_path = Path(config.bronze_configuration.columns_to_clean_rules)
    with open(cleaning_rules_path, "r") as f:
        return json.load(f)
...
def clean_col_names(
    ...
    cleaning_rules = _load_cleaning_specs(config)
