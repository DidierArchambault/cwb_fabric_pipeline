...
def bronze_lunch(ctx: RunContext, config: Configuration, pipeline_run_id:str) -> None:
    ...
    if ctx.work == "all":
        ...
        try:
            ...
            df_cleaned = _guard("bronze_cleansing", lambda: clean_col_names(config, df_raw))
            df_renamed = _guard("bronze_cleansing", lambda: rename_cols(config, df_cleaned))
            df_technicals = _guard("bronze_add_technicals", lambda: add_technicals(ctx, config, df_renamed, pipeline_run_id))
